...
/*   Updated: 2023/11/16 00:02:42 by vshchuki         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_read_chunk(int fd, char **cache, t_state *s_store)
{
	...
	{
		s_store->t = ft_strdup_s(*cache);
		free(*cache);
		*cache = ft_strjoin(s_store->t, s_store->buffer);
		free(s_store->t);
	}
	free(s_store->buffer);
	return (0);
...
int	ft_handle_cache(char **cache, t_state *s_store)
{
	char	*t;

	s_store->result = ft_substr(*cache, 0,
			ft_strchr(*cache, '\n') - *cache + 1);
	if (!s_store->result)
		return (-1);
	t = ft_strdup_s(*cache);
...
int	ft_handle_read(int fd, char **cache, t_state *s_store)
{
	...
	if (r == -1)
		...
		s_store->result = ft_strdup_s(*cache);
	if (r == -1 || r == 2)
		return (-1);
	return (0);
}
...
char	*get_next_line(int fd)
{
	...
	while (1)
	{
		...
		{
			if (ft_handle_cache(&cache, &s_store) == -1)
				break ;
			else
